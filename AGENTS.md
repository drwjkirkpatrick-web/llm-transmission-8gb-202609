...
- **jetson-model-zoo** (~/projects/jetson-model-zoo/): 27 models, temp sweep (7 values) + top_k sweep (3 values), 12 prompts, 2916 scored rows. Source for quality scores and optimal params.
...
2. **k=40 is the confirmed optimal top_k** for 19/27 models. Don't change the default without running a new sweep.
3. **Temperature is locked per-model** in `best_temps.json`. Don't use a blanket temp.
4. **Thinking models need --jinja flag.** Without it, thinking models (DeepSeek R1, SmallThinker, Ministral Reasoning, Qwen3.5, Phi4-mini) output <50 tokens.
5. **7B+ thinking models need reduced context** (-c 2048) on 8GB Jetson to avoid OOM.
6. **Quality before speed.** The routing table is sorted by quality, not speed. Speed is shown for the operator to make tradeoffs.
